...
; Lab Assignment #5 Part 1 (Array Index Fibonacci Sequence)
...
mov esi, 0
mov eax, esi
mov sequence[esi], 0 ; f(0) = 0
call printResult
inc eax
mov ebx, type sequence ; DWORD = 4 bits
add esi, ebx
mov sequence[esi], 1 ; f(1) = 1
call printResult
mov ecx, 39
Fibonacci:
	inc eax
	sub esi, ebx
	mov edx, sequence[esi]
	add esi, ebx
	add edx, sequence[esi]
	add esi, ebx
	mov sequence[esi], edx ; f(n) = f(n-2) + f(n-1)
	push eax
	mov eax, 50
